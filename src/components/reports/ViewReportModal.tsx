...
export const ViewReportModal: React.FC<ViewReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-3xl my-6 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-4 sm:p-8 space-y-6 overflow-y-auto flex-1 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 print:bg-white print:text-slate-900 print:p-0 print:overflow-visible">
          
          {/* Header Letterhead */}
          <div className="border-b-2 border-blue-600 dark:border-slate-700 print:border-slate-900 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-lg sm:text-xl font-black uppercase tracking-wider text-blue-950 dark:text-white print:text-slate-900">
            ...
            <div className="text-left sm:text-right">
              ...
              <p className="text-xs font-mono font-bold mt-1 text-slate-700 dark:text-slate-300 print:text-slate-700">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 print:bg-slate-50 border border-slate-200 dark:border-slate-800 print:border-slate-200 rounded-xl p-4 grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
            <div>
              <p className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 print:text-slate-500">Patient Name</p>
              <p className="font-extrabold text-slate-900 dark:text-white print:text-slate-900 text-sm">{report.patientName}</p>
            </div>
            <div>
          ...
          {/* Test Name Header */}
          <div className="bg-blue-600 dark:bg-slate-800 print:bg-slate-900 text-white p-3.5 rounded-xl flex items-center justify-between shadow-xs">
            <h3 className="text-xs sm:text-sm font-bold tracking-wide uppercase">
              {report.testName}
            </h3>
            <span className="text-[11px] sm:text-xs text-blue-100 dark:text-slate-300 print:text-slate-300">
              Diagnostic Report
            </span>
          </div>
          ...
          {report.mode === 'Generated' && report.parameters && (
            <div className="border border-slate-300 dark:border-slate-800 print:border-slate-300 rounded-xl overflow-x-auto">
              <table className="w-full text-left text-xs min-w-[500px]">
                <thead className="bg-slate-100 dark:bg-slate-800/80 print:bg-slate-100 text-slate-800 dark:text-slate-200 print:text-slate-800 font-bold uppercase text-[10px] border-b border-slate-300 dark:border-slate-800 print:border-slate-300">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800 print:divide-slate-200">
                  {report.parameters.map(param => (
                    <tr key={param.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 print:hover:bg-transparent">
                      <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white print:text-slate-900">{param.name}</td>
                      ...
                      <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400 print:text-slate-600">{param.unit}</td>
                      <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400 print:text-slate-600">{param.normalRange}</td>
          ...
          {report.mode === 'Uploaded' && (
            <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 print:border-slate-300 rounded-2xl p-6 bg-slate-50 dark:bg-slate-800/40 print:bg-slate-50 text-center space-y-3">
              <div className="w-12 h-12 mx-auto rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-sky-400 flex items-center justify-center">
              ...
              {report.notes && (
                <div className="text-xs text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 print:bg-white print:text-slate-700 p-3 rounded-xl border border-slate-200 dark:border-slate-700 text-left">
          ...
          {(report.impression || report.recommendations) && (
            <div className="space-y-3 bg-slate-50 dark:bg-slate-800/50 print:bg-slate-50 p-4 rounded-xl border border-slate-200 dark:border-slate-800 print:border-slate-200">
              {report.impression && (
                <div>
                  <h4 className="text-xs font-bold uppercase text-slate-900 dark:text-white print:text-slate-900 tracking-wider">
                    Clinical Impression:
                  </h4>
                  <p className="text-xs text-slate-800 dark:text-slate-200 print:text-slate-800 mt-1 font-medium leading-relaxed">
                    {report.impression}
                  </p>
                </div>
              )}
              ...
              {report.recommendations && (
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 print:border-slate-200">
                  <h4 className="text-xs font-bold uppercase text-slate-900 dark:text-white print:text-slate-900 tracking-wider">
                    Physician Advice / Recommendations:
                  </h4>
                  <p className="text-xs text-slate-800 dark:text-slate-200 print:text-slate-800 mt-1 leading-relaxed">
                    {report.recommendations}
                  </p>
                </div>
              )}
          ...
          {/* Footer Signature */}
          <div className="pt-8 border-t border-slate-300 dark:border-slate-800 print:border-slate-300 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <div className="text-center w-48">
              <div className="border-b border-slate-400 dark:border-slate-600 pb-1 mb-1 text-slate-800 dark:text-slate-200 font-serif italic text-sm">
                {report.technicianName || 'Dr. Sarah Jenkins'}
              </div>
              <p className="text-[11px] font-bold text-slate-900 dark:text-white">Medical Technologist / Pathologist</p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400">Authorized Signatory</p>
            </div>
          </div>
