  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-emerald-600 dark:bg-emerald-500 text-white font-bold text-xs hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors flex items-center gap-2"
            >
        ...
        {/* Status Tab Bar */}
        <div className="flex items-center gap-1 border-t border-slate-100 pt-3 text-xs font-semibold overflow-x-auto">
      ...
      <div className="space-y-3">
        {filteredInquiries.map(inq => (
          <div
            ...
            className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs hover:border-teal-500/40 hover:shadow-md transition-all cursor-pointer space-y-3"
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 line-clamp-2 mt-1">{inq.description}</p>
            </div>

            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <div className="flex items-center gap-4 flex-wrap">
                <span>
                  Patient: <strong className="text-slate-800">{inq.patientName}</strong>
                </span>
                {inq.relatedDoctorName && (
                  <span>
                    Doctor: <strong className="text-slate-800">{inq.relatedDoctorName}</strong>
                  </span>
                )}
                {inq.rxNumber && (
                  <span className="font-mono">
                    Rx#: <strong className="text-slate-800">{inq.rxNumber}</strong>
                  </span>
                )}
              </div>
          ...
          <div className="space-y-5 text-xs">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-sm">{selectedInquiry.subject}</h3>
                <div className="flex items-center gap-2">
                  <Badge
                    variant={
              ...
              <p className="text-slate-600">{selectedInquiry.description}</p>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {selectedInquiry.notes.map(note => (
                  <div key={note.id} className="p-3 rounded-xl bg-slate-100/80 border border-slate-200/60 space-y-1">
              ...
              <form onSubmit={handleAppendNote} className="mt-3 flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                />
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-teal-600 text-white font-bold hover:bg-teal-700 shrink-0 flex items-center gap-1"
                >
      ...
      >
        <form onSubmit={handleInquirySubmit} className="space-y-3 text-xs">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Detailed Clinical Context</label>
            <textarea
              rows={3}
              value={description}
